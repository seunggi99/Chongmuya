...
- **AI로 빠르게 구현하고, 사람이 판단해야 할 지점에 개입**했습니다 — 설계 결정, 핵심 회계 로직, 막힌 디버깅.
- 그래서 2일이라는 짧은 기간에도 실제로 쓸 수 있는 완성도까지 끌어올렸습니다.
...
실제 산악회가 엑셀로 수동 관리하던 경비 일지를 분석해 도메인 요구사항을 도출했습니다.   
(이월금 옮겨 적기, 잔액 손계산, 은행내역 타이핑, 교차 회차 수기 분리 등 반복·오류 지점을 자동화)
...
> AI로 빠르게 만들고, **설계 결정과 막힌 지점은 직접 파고들어 해결**했습니다.

**워크플로우**
- **설계 먼저, 구현은 위임** — `claude.md`를 단일 진실 공급원으로 두고 스키마·로직·컨벤션을 문서화한 뒤 그 명세로 구현
- **작업을 작게 쪼개 관리** — 빌드·타입체크·ESLint 통과 시에만 커밋해 산출물을 일정 품질로 유지

**직접 판단·설계한 핵심**
- **교차 회차 이중 집계** *(이 프로젝트의 핵심 난제)* — 선입금/선지급을 별도 테이블이 아닌 `entries.cross_session_id`로 모델링해, 같은 항목을 통장 뷰엔 *발생 회차* / 결산 뷰엔 *귀속 회차*로 집계. 잔액 공식을 `lib/balance.ts` 한 곳에 모아 두 관점의 정합성 보장
- **데이터 정합성** — 저장 시 서버 재검증(`상세 합 = entry.amount`, `분할 합 = 원본`), 다중 행 저장 실패 시 보상 롤백(`lib/sessionsSave.ts`)으로 부분 저장 방지
- **확장성** — 분류·행사 유형·직책·연회비 규칙을 코드 상수가 아닌 DB/설정으로 일반화(다른 모임에 코드 수정 없이 제공)

**막혔을 때 직접 추적해 해결한 사례**
- **결산 잔액 버그** — `bigint` 문자열 직렬화(`0 + "5000" = "05000"`)가 원인임을 추적해 숫자 변환을 한 곳(`lib/balance.ts`)에서 처방
- **Vercel PDF 실패** — 1차 외부화로 안 풀리자 추측을 멈추고 빌드 산출물 **파일 트레이스(`.nft.json`)를 직접 분석**해 chromium 바이너리(`bin/*.br`) 누락 확정 → `outputFileTracingIncludes`로 해결
- **잠긴 엑셀 복호화** — "비번이 맞는데 실패"를 포착하고 SheetJS 내장 복호화 한계를 확인해 `officecrypto-tool` 선복호화로 전환
- **JPG 한글 정렬 깨짐** — `html2canvas`의 한글 베이스라인 어긋남을 발견하고 서버 puppeteer 렌더로 전환해 PDF와 결과 일치
